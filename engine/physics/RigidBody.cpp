...
void RigidBody::updatePhysicsState() {
    // std::max(std::min((_velocity + _acceleration * Time::deltaTime()).y(), -100.0), 100.0)
    // _velocity = Vec3D((_velocity + _acceleration * Time::deltaTime()).x(), std::min(std::max((_velocity + _acceleration * Time::deltaTime()).y(), -20.0), 20.0), (_velocity + _acceleration * Time::deltaTime()).z());
...
Vec3D RigidBody::_support(const std::shared_ptr<RigidBody>& obj, const Vec3D &direction) {
...
std::pair<bool, Simplex> RigidBody::checkGJKCollision(std::shared_ptr<RigidBody> obj) {
    ...
    while (iters++ < size() + obj->size()) {
        ...
        if (support.dot(direction) <= 0) {
            return std::make_pair(false, points);       // No collisions
        }
        ...
        if(std::abs(sDistance - minDistance) > Consts::EPA_EPS) {
            ...
            for (auto &normal : normals) {
                if(normal.normal.dot(support) > 0) {
                    uniqueEdges = _addIfUniqueEdge(uniqueEdges, faces, f, f + 1);
