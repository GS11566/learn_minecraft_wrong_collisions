...
std::vector<Triangle> Camera::project(const std::shared_ptr<Mesh>& mesh) {
    ...
    for (auto &t : mesh->triangles()) {
        ...
        Triangle modelTri = t * M;
        Triangle modelViewTri = t * M * V;

        double dot = ((Vec3D(modelTri[0]) - position()).normalized().dot(modelTri.norm()));
        ...
        clippedTriangles.emplace_back(modelViewTri);
        ...
        for (auto &clippedTri : clippedTriangles) {
            projected = clippedTri * _SP;

            Triangle projected_normalized = Triangle(projected[0] / projected[0].w(),
                                                     projected[1] / projected[1].w(),
                                                     projected[2] / projected[2].w());

            double dotColor = (0.4 * std::abs(dot) + 0.6);
            projected_normalized.setColor(
                    sf::Color(
                            (int) (projected.color().r * dotColor),
                            (int) (projected.color().g * dotColor),
                            (int) (projected.color().b * dotColor)
                    ));
