...
void Minecraft::start() {
    ...
    camera->translateToPoint(player->position() + Vec3D(0, 3, 0));

    player->attach(camera);                     // Attach camera to player

    player->translate(Vec3D(0, 100, 0));

    // adding cube in hand
    auto cube_in_hands = world->addBody(std::make_shared<RigidBody>(Mesh::Cube(ObjectNameTag("cube_in_hand"), 2)));
    cube_in_hands->setCollider(false);
    cube_in_hands->translateToPoint(player->position() + Vec3D(-3, -1, 1.4));
    cube_in_hands->rotate(Vec3D(0, M_PI / 10, 0));

    camera->attach(cube_in_hands);
